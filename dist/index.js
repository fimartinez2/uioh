...
var ColorModeContext = React__namespace.createContext(
  null
);
function UiOhProvider({
  ...
  const setMode = React__namespace.useCallback(
    (next) => {
  ...
  const themeValue = React__namespace.useMemo(() => {
    const base = tokensByMode[currentMode];
    return deepMerge(base, theme);
  }, [currentMode, theme]);
  const colorModeValue = React__namespace.useMemo(
    () => ({ mode: currentMode, setMode, toggle }),
    [currentMode, setMode, toggle]
  );
  return /* @__PURE__ */ jsxRuntime.jsx(ColorModeContext.Provider, { value: colorModeValue, children: /* @__PURE__ */ jsxRuntime.jsx(ThemeContext.Provider, { value: themeValue, children }) });
}
function useTheme() {
  const ctx = React__namespace.useContext(ThemeContext);
  if (!ctx) throw new Error("useTheme must be used within <UiOhProvider />");
  return ctx;
}
function useColorMode() {
  const ctx = React__namespace.useContext(ColorModeContext);
  if (!ctx)
...
function Text({
  ...
  return React__namespace.createElement(as, { className, style: finalStyle }, children);
}
function Heading({
  children,
  variant = "h1",
  ...
  return React__namespace.createElement(tag, { className, style: finalStyle }, children);
}

exports.ColorModeToggle = ColorModeToggle;
exports.Heading = Heading;
exports.Text = Text;
exports.UiOhProvider = UiOhProvider;
